...
export function InDonDatHang({ data, handleClose }: InDonDatHangProps) {
  ...
  const tongTienHang = data.phieuDatHangDetail.reduce((sum, item) => {
    const soLuong = Number(item.soLuongGDDuyet) || 0;
    const donGia = Number(item.donGia) || 0;
    return sum + soLuong * donGia;
  }, 0);

  const taxGroups = data.phieuDatHangDetail.reduce(
    (acc, item) => {
      const soLuong = Number(item.soLuongGDDuyet) || 0;
      const donGia = Number(item.donGia) || 0;
  ...
  const tienThue5 = round(taxGroups[0.05] || 0);
  const tienThue8 = round(taxGroups[0.08] || 0);
  const tienThue10 = round(taxGroups[0.1] || 0);

  const tongTienThue = round(tienThue5 + tienThue8 + tienThue10);
  const tongThanhToan = round(tongTienHang + tongTienThue);
    ...
    <>
      ...
      >
        ...
        <Box textAlign="right" mb={2}>
          Ngày {ngay.getDate()} tháng {ngay.getMonth() + 1} năm {ngay.getFullYear()}
        </Box>
        <Box mb={1}>
        ...
        >
          <Box>Thời hạn giao hàng: Trong vòng</Box>
          ...
          <TextField
            ...
            disabled={data.thoiGianGiaoHang != null}
          ...
          <Box>ngày làm việc kể từ ngày ký đơn đặt hàng.</Box>
        </Box>
